...
(() => {
  ...
  class FS extends Thing {
    ...
    async import(url) {
      let response = await this.request("POST", "/fs/import", {
        url
      })
      if (response.error) {
    ...
    async push(cid) {
      let response = await this.request("POST", "/fs/push", {
        cid
      })
      if (response.error) {
        throw new Error(response.error)
...
document.addEventListener("DOMContentLoaded", async () => {
  let account = await rarepress.init({
    host: "https://eth.rarenet.app/v1"
  })
